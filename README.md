...
- `leverage_rate`: optional integer percentage from 0 through 99. It sets the account's borrowing budget to that percentage of total base equity across every account.
- `leverage_amount`: optional non-negative USD borrowing amount, mutually exclusive with `leverage_rate`. Across the portfolio, at most one account may have a positive rate or amount. Borrowing is drawn only when purchases take that account below zero cash.
- `leverage_mode`: `"max"` (default) caps actual borrowing at the configured amount or portfolio-based percentage; `"min"` requires actual borrowing to reach at least that amount. Minimum mode requires a rate or amount, overrides normal account placement priority when needed, and may exceed its threshold by the final whole-share purchase. A zero threshold requires no borrowing.
- `blocked_assets`: optional ticker list that prevents this account from receiving new purchases of those exact assets. Existing shares remain unchanged and continue to count toward portfolio targets. Blocked satellite purchases leave buying power available for the subsequent core allocation; blocked core purchases can leave cash uninvested.
- `fixed_assets`: optional current share counts. Decimal shares are accepted.

All configured percentages use TOML integers; decimal percentage literals are rejected. Unknown fields and invalid values are rejected. Total portfolio base equity must be positive; borrowing alone does not create equity. Required prices must be finite and positive, and a price failure aborts the complete allocation.
...
2. Calculate the one permitted account's configured borrowing threshold: either `leverage_rate` times total portfolio base equity or the fixed `leverage_amount`. It increases that account's initial modeled buying power without appearing as positive cash. Purchases beyond cash create debt; `max` caps it, while `min` may cross the threshold as described below.
...
5. For each ticker, skip accounts whose `blocked_assets` contains that ticker. Among eligible accounts, first visit those that already hold either the target or one of its substitutes, preserving their TOML order, then visit all other accounts in TOML order. Broker declaration order comes first, followed by account declaration order within each broker. Buy as many whole shares as the current account can hold before moving on.
6. In `"min"` mode, reach the minimum at the account boundary during ordinary allocation: round the borrower's remaining share capacity up instead of down, allowing the current asset's last whole share to cross the threshold. Never exceed the requested share count for that asset. Once debt reaches the minimum, this account receives no further purchases, including cheaper assets or assets it already holds. Its remaining buying power is zero, so overshoot does not reduce other accounts' core pool. This preserves the declared asset order and avoids reopening the borrower for a second split asset; existing positions and account restrictions can still require other splits.
7. If ordinary allocation cannot reach the minimum, reassign only this run's new purchases from other accounts, following asset/account order and rounding the final lot up to the threshold. Preserve all fixed holdings and respect blocked tickers. If still insufficient, buy extra whole shares of unblocked, positive-weight targets, preferring positions already held by the borrower. Within that preference, minimize the change in summed squared dollar deviations from original satellite and renormalized core targets (including substitutes), then overshoot, then declaration order. Stop immediately at the minimum. This is deterministic, not globally optimal; excess borrowing is less than the final share's price. With no eligible asset, fail before emitting a report. Asset prices never replace declaration order as allocation priority.
8. Never sell a surplus position and never create a fractional purchase. Uninvested cash remains in its account; borrowing appears as negative cash in the leveraged account. Both appear in the report. In minimum mode, other accounts may retain cash even while the borrower meets its debt requirement.
...
Target purchase counts are independently rounded down to whole shares. Their combined remainders stay as cash, even when enough cash remains to buy a cheaper asset. There is no final cash sweep or automatic overweight purchase to spend those remainders; minimum-borrowing enforcement is the explicit exception described above.
...
The full Markdown report shows a `Portfolio` section first, followed by each account in configuration order. Portfolio tables aggregate shares and values by actual ticker across all accounts and show portfolio percentages; account tables retain both account and portfolio percentages. Substitutes remain separate ticker rows with their `Counts Toward` target, so shares of different ETFs are never combined. Each level has a compact equity and cash summary. The portfolio summary also shows gross exposure, and each account summary shows its borrowing budget, actual debt, and blocked purchases when configured. Broker and account snake_case names are converted to title case. ETF rows include current prices, asset names, expense ratios and categories from `funds_data`, and substitution targets from configuration; equity rows include prices, asset names, market capitalization, and one-year price change from `fast_info`. Optional metadata failures display `—` and do not abort allocation.
